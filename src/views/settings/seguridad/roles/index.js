...
import { getRoles, createRol, getRolById, updateRol, deleteRol } from 'api/seguridad/rolesApi';
...
const Roles = () => {
    ...
    const handleItemCreated = async (values) => {
        const resp = await createRol(values);
        if (!resp.success) {
            toast.error(resp.responseData.message ?? resp.errorMessage);
    ...
    const handleItemEdit = async (id) => {
        const resp = await getRolById(id);
        if (!resp.success) {
            toast.error(resp.responseData.message ?? resp.errorMessage);
            return;
        }
    ...
    const handleItemUpdate = async (values) => {
        const resp = await updateRol(values.id, values);
    ...
    const handleDialogConfirmDelete = async () => {
        const resp = await deleteRol(itemIdToDelete);
        if (!(resp === '')) {
            toast.error(resp.responseData.message ?? resp.errorMessage);
            return;
        }
    ...
    return (
        <MainCard style={{ marginTop: '20px' }}>
            <Grid container spacing={2} sx={{ p: 2 }}>
                ...
                <Grid item xs={12}>
                    <RolesList data={data} onEdit={(id) => handleItemEdit(id)} onDelete={handleItemDelete} />
                </Grid>
            </Grid>

            <RolesForm open={openForm} handleClose={handleFormClose} onSubmit={selectedItem ? handleItemUpdate : handleItemCreated} initialValues={selectedItem || {}} />
